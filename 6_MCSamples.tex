...
Apart from the energy scale, another important feature of the simulated electrons is the shower shape variables which are used for electron identification. It is especially important for forward electrons, since with absence of the tracker information the shapes are the only discriminatory variables that help to identify the electrons (see Sec.~\ref{sec:Rec_elecID} for details). The comparison of the shower shape variables can be seen in Figs.~\ref{fig:MC_fs_shape1} to~\ref{fig:MC_fs_shape6}.

\begin{figure}
\center{
\includegraphics[width=0.8\textwidth]{figures/fsshape_lambda_centre.pdf}
\caption{The shower depth variable. The distance of the shower barycenter from the calorimeter front face measured along the shower axis.}
...
\caption{Maximum cell energy variable. Fraction of the cluster energy in the most energetic cell.}
...
\caption{Longitudinal second momentum. Second momentum of the distance of each cell to the shower center in the longitudinal direction ($\lambda_i$).}
...
\caption{Transverse second momentum. Second momentum of the distance of each cell to the shower center in the transverse direction ($r_i$).}
...
\caption{Normalized lateral momentum. $w_\mathrm{norm} = \frac{w_2}{w_2+w_\mathrm{max}}$ where $w_2$ and $w_\mathrm{max}$ are second momenta of $r_i$ for different weights per cell.}
...
\caption{Normalized longitudinal momentum. $\lambda_\mathrm{norm} = \frac{\lambda_2}{\lambda_2+\lambda_\mathrm{max}}$ where $\lambda_2$ and $\lambda_\mathrm{max}$ are second momenta of $\lambda_i$ for different weights per cell.}
...
\caption{The comparison of the $Z$ boson $p_T$ distribution for various MC generators. Distributions are normalized to arbitrary scale.}
\label{fig:MC_zpt}}
\end{figure}
